...
memberEmail.addEventListener("input", e => {
    ...
    .then(resp=>resp.text())
    .then(count=>{
        //count:1이면 중복, 0이면 중복아님
        if(count==1){ //중복 ㅇ
            emailMessage.innerText="이미 사용중이야";
...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    .then(result=> {
        if(result==1) {
        console.log("인증번호 발송성공");
        } else {
        console.log("인증번호 발송실패");
        }
    })
