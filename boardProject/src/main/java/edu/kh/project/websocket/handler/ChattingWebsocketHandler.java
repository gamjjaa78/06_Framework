...
public class ChattingWebsocketHandler extends TextWebSocketHandler {
	...
	// 클라로부터 텍스트 메시지 받았을때 실행하는 메서드
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {

		// message - JS에서 클라로부터 전달받은 내용
		...
		if (result > 0) {
			...
			// 필드에 있는 sessions에는 채팅페이지에 접속한 로그인한 모든 회원의 세션이 있음
			// ->이 sessions에서 현개 로그인한 회원, 대상을 구해야함
			for (WebSocketSession s : sessions) {
				...
				if (loginMemberNo == msg.getTargetNo() || loginMemberNo == msg.getSenderNo()) {
					// 메세지 전달(서버쪽에서 해당되는 클라에게 해당 메세지를 전송)
					// java(message DTO)->js(JSON변환:js에 보내야 하니까)
					String jsonDate = objectMapper.writeValueAsString(msg);
					s.sendMessage(new TextMessage(jsonDate));
				}
			}
