...
public class BoardServiceImpl implements BoardService {
	...
	public Map<String, Object> selectBoardList(int boardCode, int cp) {
		...
		// 2.1번의 결과+cp를 이용해서
		// Pagination 객체생성:게시글 목록 구성에 필요한 값을 저장한 객체
		...
		// Mapper 메서드 호출시 원래 전달할 수 있는 매개변수 1개
		// 2개를 전달할 수 있는 경우가 있음->RowBounds 이용시
		// 1번째 매개변수->SQL에 전달할 파라미터
		// 2번째 매개변수->RowBounds 객체, 자리정해짐, 무조건 2번째 자리
	...
	public Map<String, Object> searchList(Map<String, Object> paramMap, int cp) {
		// 1.지정된 게시판(boardCode)에서 검색조건에 맞으면서
		// 삭제되지 않은 게시글 수를 조회
	...
	public Board selectOne(Map<String, Integer> map) {
		//여러 SQL을 실행하는 방법
		//1. 하나의 Service 메서드에서 여러 mapper 메서드 호출하는 방법
		
		//2.수행하려는 SQL이
		//모두 SELECT 이면서
		//먼저 조회된 결과 중 일부를 이용해
		//나중에 수행되는 SQL의 조건으로 삼을 수 있는 경우
		//->Mybatis의 <resultMap>, <collection>태그이용해
		//Mapper 메서드 1회 호출만으로 여러 SELECT 한번에 수행가능
