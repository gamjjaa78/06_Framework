...
public class FileConfig implements WebMvcConfigurer {
	...
	public void addResourceHandlers(ResourceHandlerRegistry registry) {
		...
		//Spring MVC에서 정적리소스(이미지, css, js 등)의 요청처리위한 클래스
	...
	public MultipartConfigElement configElement() {
		...
		//파일 업로드 처리하는데 사용되는 MultipartConfigElement를 구성하고 반환(옵션 설정시 사용)
	...
	//MultipartResolver 객체 생성해 Bean으로 등록
	//위에서 만든 MultipartConfigElement 자동으로 이용함
	@Bean
	public MultipartResolver multipartResolver() {
		 //MultipartResolver MultipartFile을 처리해주는 해결사
		//MultipartResolver는 클라로부터 받은 multipart 요청 처리,
		//그중 업로드된 파일 추출해 MultipartFile 객체 제공하는 역할
