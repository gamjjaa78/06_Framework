...
@Slf4j //log를 이용한 메시지 콘솔창에 출력할때 사용(lombok 제공)
public class parameterController {
	@GetMapping("main")
	public String paramMain() { // /param/main  Get 방식 요청 매핑
		// src/main/resources/template/param/param-main.html로 forward
		//접두사 classpath"/templates/
		//접미사 .html
		return "param/param-main"; //접두,접미사 뺀 나머지
	}
	
	//HttpServletRequest
	//요청 클라이언트의 정보, 제출된 파라미터 등을 저장한 객채
	//클라이언트 요청시 상성
	
	//Spring 의 Controller 단 메서드 작성시
	//매개변수에 원하는 객체 작성시
	//존재하는 객체를 바인딩 또는 없으면 생성해서 바인딩
	//->ArgumentResolver (전달 인자 해결사)
	
	@PostMapping("test1") // /param/test POST 방식 요청 매핑
	public String paramTest(HttpServletRequest req) {
	...
	public String paramTest2(@RequestParam("title") String title, 
							...
							//매개변수의 자동형변환이 사용된 경우에는 파라미터값이 필수로 작성돼야함
	...
	//제출된 파라미터를 한번에 묶어서 얻어오기(Map<String, Object>)
	@PostMapping("test3")
	public String paramTest3(@RequestParam("color") String[] colorArr,
		...
		//같은 name 속성가진 파라미터는 배열이나 List 형태가 아님
	...
	//전달받은 파라미터의 name 속성값이 함께 사용되는 DTO의 필드명이 같다면
	//자동으로 setter를 호출해서 필드에 값저장
	
	//****주의사항****
	//DTO에 기본생성자가 필수로 존재해야함
	//DTO에 setter가 필수로 존재해야함
	
	//@ModelAttribute를 이용해 값이 필드에 세팅된 객체를 "커맨드 객체"라함
	//@ModelAttribute 생략가능!!
	@PostMapping("test4")
	public String paramTest4(/*@ModelAttribute*/ Member inputMember) {
		...
		*/
		...
		return "redirect:main"; //상대경로작성법, 현재위치 중요!!!!
						//현재 경로의 가장 마지막 레벨의 주소값을 "redirect:이주소"로 변경하기 때문
						//"redirect:/param/main" <절대경로
	}
